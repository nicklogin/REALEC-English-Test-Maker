...
##                print('Stanford POS Tagger OK')
    ...
    def create_multiple_choice_ex(self, sent, wrong, right_answer, lb, rb):
        new_sent = sent[:lb] + "_______ " + sent[rb:]
    ...
    def create_sentence_function(self, new_text, lengths):
        ...
        if self.context:
            ...
            sentences = [''.join((self.correct_all_errors(sentences[i]),sentences[i+1],
            self.correct_all_errors(sentences[i+2]))) for i in range(0,len(sentences),3) if '<<' in sentences[i+1]]
            ...
            if '<<' in sent2:
                ...
                if sent2.count('<<') == 1:
                    ...
                    if ex_type == 'multiple_choice':
                        new_sent, answers = self.create_multiple_choice_ex(sent2, wrong, right_answer, lb, rb)
                        if not answers:
                            if len(types2) > 0:
                                ex_type = random.choice(types2)
                            else:
                                continue
                                    ...
                                    if self.make_two_variants:
                                        if i==chosen2:
                                            if ex_type == 'short_answer':
                                                ...
                                                answers2 = self.find_choices(right_answer, wrong, new_sent2)
                                                if len(answers)<3:
                                                    new_sent2 += right_answer + other
                                                    answers2 = []
                                        else:
