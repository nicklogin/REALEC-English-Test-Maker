...
def find_prep(phrase):
    ...
    for i in range(len(phrase)):
        if phrase[i].lower() in prep_list:
            l = ' '.join(phrase[:i])+' '
            try:
                r = ' '+' '.join(phrase[i+1:])
            except:
                r = ''
            return {'prep':phrase[i], 'left':l, 'right':r}
    return {'prep':'', 'left':' '.join(phrase), 'right':''}
...
def find_verb_form(phrase, formname):
    phrase = phrase.split(' ')
    form = ''
    ##в этом случае возвращаем последнюю синтетическую форму в составе
    ##первой встреченной аналитической
    if formname == 'any':
    ...
    ##в этом случае находим первую попавшуюся синтетическую форму заданного
...
def find_anal_form(phrase, synth_form):
    phrase = phrase.split(' ')
    be_forms = [val for key,val in vff.find_verb_forms('be').items()]
